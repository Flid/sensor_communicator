...
URL_BASE = 'https://api.mobile.endomondo.com'
URL_AUTHENTICATE = URL_BASE + '/mobile/auth'
URL_WORKOUTS = URL_BASE + '/mobile/api/workouts'
URL_WORKOUT_GET = URL_BASE + '/mobile/api/workout/get'
URL_WORKOUT_POST = URL_BASE + '/mobile/api/workout/post'
URL_TRACK = URL_BASE + '/mobile/track'
URL_PLAYLIST = URL_BASE + '/mobile/playlist'

URL_ACCOUNT_GET = URL_BASE + '/mobile/api/profile/account/get'
URL_ACCOUNT_POST = URL_BASE + '/mobile/api/profile/account/post'
...
class MobileApi(object):
    ...
    device_info = {
        ...
        'appVariant':    'SensorsCommunicator',
        'country':        'GB',
    ...
    def __init__(self, **kwargs):
        ...
        self.Requests.headers['User-Agent'] = (
            '{appVariant}/{appVersion} ({os}; {osVersion}; {model}) {vendor}'.format(
                **self.device_info
            )
        )
    ...
    def request_auth_token(self, email, password):
        ...
        if lines[0] != 'OK':
            ...
            logging.warning(
                'Logging failed into Endomondo, returned data was: %s',
                r.text,
            )
            raise AuthenticationError(
                'Could not authenticate with '
                'Endomondo, Expected \'OK\', got \'%s\'' % lines[0]
            )
    ...
    def make_request(self, url, params=None, method='GET', data=None, **kwargs):
        """
        ...
        if r.status_code != requests.codes.ok:
            logging.debug(
                'Endomondo returned failed status code. Code: %s, message: %s',
                r.status_code,
                r.text,
            )
        ...
        try:
            ...
            if data.has_key('error'):
                logging.warning(
                    'Error loading data from Endomondo. Type: %s',
                    data['error'].get('type'),
                )
    ...
    def post_account_info(self, account_info=None, **kwargs):
        """
         Save user info.
        ...
        if account_info.has_key('date_of_birth'):
            account_info['date_of_birth'] = datetime_to_str(
                account_info.get('date_of_birth'),
            )
    ...
    def get_workouts(self, before=None, **kwargs):
        ...
        :param before: Optional datetime object or iso format
               date string (%Y-%m-%d %H:%M:%S UTC)
        :param maxResults: Optional Maximum number of
               workouts to be returned. Default is 20
        :param deflate: Optional true or false. Default
               is false, whereas endomondo app default is true. Untested.
        :param fields: Optional Comma separated list for
               endomondo properties to return.
               Default is: device,simple,basic,lcp_count
        """
        ...
        if before is not None:
            if isinstance(before, datetime):
                ...
                raise ValueError(
                    'Param \'before\' needs to be datetime '
                    'object or iso formatted string.'
                )
        ...
        if data.has_key('error'):
            ...
            if err_type == 'NOT_FOUND':
                raise NotFoundException(
                    'Item ``%s`` was not found in Endomondo.' % kwargs['workoutId'],
                )
            else:
                raise EndomondoException(
                    'Error while loading data from Endomondo: %s' % err_type,
                )
    ...
    def post_workout(self, workout, properties=None):
        """
        ...
        # will complain about missing TrackPoints
        # (not directly related to ``Workout.TrackPoint``)
        properties.setdefault('gzip', 'true')

        if not workout.device['workout_id']:
            workout.device['workout_id'] = random.randint(
                -9999999999999999999,
                9999999999999999999,
            )
        ...
        for chunk in chunks(workout.points, 100):
            ...
            if lines[0] != 'OK':
                raise EndomondoException(
                    'Could not post track. Error ``%s``. '
                    'Data may be partially uploaded' % lines[0]
                )
